...
Running `kubectl-mcp-server` from a remote pod depends on a live cluster,
credentials, and venue wifi - too fragile for a 15-minute conference slot
with nothing to fall back on if the network blinks.

This repo makes `kubectl-mcp-server` fully local and self-contained: one
`./setup.sh` in the green room creates a disposable `kind` cluster, installs
a correctly-pinned `kubectl-mcp-server`, and wires it into Claude Code - no
remote dependency of any kind once the images are cached.
...
NetworkPolicy while still blocking a delete. The server here runs
unrestricted; the actual safety boundary is two scripted, reviewed prompts
that never ask the AI to delete anything.

## How `setup.sh` wires this into Claude Code

`setup.sh` registers a `kubernetes` MCP server at user scope via
`claude mcp add-json` - Claude Code's own supported interface, not a
hand-edited config file. It's pointed at this repo's own pinned install and
its own dedicated kubeconfig, so it never touches your default kubeconfig,
current context, or any other MCP server you have configured:
...
Check it landed with `claude mcp get kubernetes`. If a Claude Code session
was already open when `setup.sh` ran, start a new one (or reload) before it
will show up in `/mcp` - MCP servers are read at session start, not live.
